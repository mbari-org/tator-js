...
export class TextOverlay extends HTMLElement {
  constructor() {
    super();
...
export class AnnotationCanvas extends HTMLElement
{
  ...
  constructor()
  {
    ...
    this.contextMenuAvailable = new Promise((resolve) => {
  ...
  cloneToNewVersion(localization, dest_version)
  {
    ...
    if (this._data._trackDb.has(localization.id))
    {
      ...
      if (track.version != this._data.getVersion().id)
      {
      ...
      {
        // We already have a track on the new version, but the box wasn't
      const track = this._data._trackDb.get(localization.id);
      const state_type = this._data._dataTypes[track.type];
